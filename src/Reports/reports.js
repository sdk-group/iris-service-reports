...
class Reports {
  ...
  actionGetTable(table) {
    ...
    _.forEach(rows, (row, index) => {
      let group = Splitters.compose(entity_name, row.group);
      let filter = Filters.compose(entity_name, row.filter);
      let aggregator = Aggregators.get(entity_name, row.aggregator);
      let groupby = row.groupby;
      let key = row.key;
      //@FIXIT: call parse only once!
      // let accumulator = {};
      //
      // source.parse((data) => {
      ...
      //     if (!accumulator[group_index]) accumulator[group_index] = [];
      //
      //     accumulator[group_index].push(data_row[key]);
      //   })
      // }).finaly(() => {
      //   result[index] = _.map(accumulator, aggregator);
      // });
